...
class SaleReportAnalysis(models.TransientModel):
    _inherit = "sale.report.analysis"

    status = fields.Selection(selection=SALE_ORDER_STATE,
        string='Status', help='Status', default='all', required=True)
    print_type = fields.Selection(
        [('sale', 'Transaction'), ('product', 'Products')],
    ...
    def _get_data(self):
        ...
        if not self.customer_ids:
            customer_ids = self.env['res.partner'].search([('user_type','=','customer'),('is_another_address','=',False)])
            self.customer_ids = customer_ids.ids
